...
var File = Object.derive(function(){
    ...
    if(realpath.indexOf(root) === 0){
       ...
       // console.log(this.url);
        ...
        if(this.release === false){
            ...
            this.url = this.url ? normalizePath(this.url.replace(/[\/\\]+/g, '/'), reg, rExt) : this.release;
        }
    ...
    getUrl : function(withHash, withDomain){
        var url = this.url;
        if(!fis.config.get('md5Query.open') && withHash && this.useHash){
        ...
        if(fis.config.get('md5Query.open') && withHash && this.useHash){
            var query, hashQuery = fis.config.get('md5Query.name') + '=' + this.getHash();
    ...
    getHashRelease : function(release){
        ...
        if(release){
            if(!fis.config.get('md5Query.open') && this.useHash){
